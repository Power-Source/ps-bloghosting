		...
		public static function admin_coupons() {
			...
			?>
				...
				<script type="text/javascript">
					document.addEventListener('DOMContentLoaded', function () {
						...
						document.querySelectorAll('.pickdate').forEach(function (input) {
							if (input.type !== 'date') {
			...
			<div class="wrap">
				...
				if ( isset( $_POST['submit_settings'] ) ) {
					...
					$coupons[ $new_coupon_code ]['valid_for_period'] = isset( $_POST['valid_for_period'] ) ? (array) $_POST['valid_for_period'] : array();
						...
						<div class="postbox">
							...
							<div class="inside">
								...
								if ( isset( $new_coupon_code ) && isset( $coupons[ $new_coupon_code ] ) ) {
									...
									$valid_for_period = isset( $coupons[ $new_coupon_code ]['valid_for_period'] ) ? (array) $coupons[ $new_coupon_code ]['valid_for_period'] : array();
								}
								?>
								<table id="add_coupon">
									<thead>
									...
									<tr>
										...
										<td>
											<input value="<?php echo $discount; ?>" size="3" name="discount"
											       type="text"/>
											<select name="discount_type" class="narrow">
												<option
										...
										if ( ! empty( $periods ) ) {
											?>
											<td>
												<select name="valid_for_period[]" class="psts-period" data-placeholder="Zeitraum auswählen">
													<option
