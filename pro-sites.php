...
//------------------------------------------------------------------------//
	...
	*/
	...
	function plug_network_pages() {
		...
		add_action( 'admin_print_scripts-' . $psts_pricing_page, array( &$this, 'load_levels_style' ) );
		...
		// Always enqueue help tooltip script on Pro Sites admin pages
		add_action( 'admin_enqueue_scripts', array( &$this, 'enqueue_help_tooltips' ) );
	...
	*/
	...
	function scripts_coupons() {
		// jQuery UI datepicker removed; native date inputs are initialized inline on the coupons page.
	}
	...
	function register_psts_style() {
		...
		// Lightweight tooltip handler that does not require jQuery
		wp_register_script( 'psts-help-vanilla', $this->plugin_url . 'js/psts-help-vanilla.js', array(), $this->version, true );
	...
	function load_psts_style() {
		...
		wp_enqueue_script( 'psts-help-vanilla' );
	}

	/**
	 * Ensure help tooltips load even if other assets are disabled.
	 */
	function enqueue_help_tooltips() {
		global $pagenow;
		if ( 'admin.php' !== $pagenow ) {
			return;
		}
		if ( empty( $_GET['page'] ) || 0 !== strpos( sanitize_text_field( $_GET['page'] ), 'psts' ) ) {
			return;
		}
		wp_enqueue_script( 'psts-help-vanilla' );
	}
	...
	if ( isset( $_POST['add_level'] ) ) {
		...
		if ( ! $error ) {
			$level_data = array(
				...
				'price_1'    => round( isset( $_POST['add_price_1'] ) ? (float) $_POST['add_price_1'] : 0, 2 ),
				'price_3'    => round( isset( $_POST['add_price_3'] ) ? (float) $_POST['add_price_3'] : 0, 2 ),
				'price_12'   => round( isset( $_POST['add_price_12'] ) ? (float) $_POST['add_price_12'] : 0, 2 ),
				'is_visible' => intval( @$_POST['add_is_visible'] ),
				'setup_fee'  => round( isset( $_POST['add_setup_fee'] ) ? (float) $_POST['add_setup_fee'] : 0, 2 ),
			);
